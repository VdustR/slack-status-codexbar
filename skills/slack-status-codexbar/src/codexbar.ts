...
function compactAbsoluteDescription(description: string): string | null {
  const monthAtTime = description.match(
    /^([a-z]+)\s+(\d{1,2})\s+at\s+(\d{1,2}):(\d{2})(?:\s*([ap]m))?$/i,
  );
  if (monthAtTime) {
    const month = monthNumber(monthAtTime[1]!);
    if (!month) return null;
    const day = Number(monthAtTime[2]);
    const hour = normalizeHour(monthAtTime[3]!, monthAtTime[5] ?? null);
    return `${month}/${day} ${hour}:${monthAtTime[4]}`;
  }
